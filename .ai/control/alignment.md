...
- [x] **Crash "Primitive.button failed to slot onto its children"** en `/residentes` y `/comunicados`: el `trigger` (`<Button>`) del diálogo se creaba en un **Server Component** (la página) y se pasaba como prop; al cruzar la frontera RSC, el `asChild` de Radix falla. Fix: `ResidentFormDialog`/`AnnouncementFormDialog` ahora crean su botón por defecto **internamente** (cliente) si no se pasa `trigger`; las páginas ya no pasan `trigger`. Regla: nunca crear un trigger `asChild` en un Server Component. (Los triggers creados en componentes cliente —ej. "Editar" en las tablas— siguen funcionando.)
